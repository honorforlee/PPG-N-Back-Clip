...
for filename_with_ext in fnmatch.filter(os.listdir(raw_ppg_data_dir), '*.txt'):
    ...
    raw_ppg_data = load_text(filename=full_filename)
...
for filename_with_ext in fnmatch.filter(os.listdir(raw_biopac_data_dir), '*.txt'):
    ...
    raw_biopac_data = load_text(filename=full_filename)
...
for participant in output_data:
    ...
    dump_json(data=output_data[participant], filename=full_output_filename, overwrite=True)